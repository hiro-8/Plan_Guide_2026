...
9. **Rain warning (`spot.rainRisk` / `isRainRiskForSpotOnDate` / `buildRainWarningBanner`)** — unlike the regex-derived tags above, whether a spot is weather-disruptive is an explicit `rainRisk: true` boolean set by hand on each relevant spot (currently `seibuen`, `sakitama`, `nagatoro`, `forest`, `shinrinkoen`, `sayamaski`) — regex-guessing from free text proved unreliable (e.g. `kawahaku` mentions "屋外の水遊び広場" but is primarily an indoor rain-safe museum). When a `rainRisk` spot's forecast for that day has ≥50% precipitation or a rain/thunderstorm weather code, an amber banner appears in the day detail (and a ⚠️ badge in the week overview) offering one-tap swaps to nearby 🏠-tagged indoor spots. Set `rainRisk: true` on any new spot where rain would meaningfully cancel/reduce the activity.
...
Known gap: several `foods[].image` entries reference files that don't yet exist in `images/` (e.g. `forest-food-*.jpg`, `hashidate-food-*.jpg`, `saiboku-food-*.jpg`, `shinrinkoen-food-*.jpg`, `gaikaku-food-2.jpg`, `tetsudouhaku-food-1.jpg`) — these are placeholder rows meant to be replaced with real restaurant picks and photos before the trip. `tetsudouhaku` (鉄道博物館), `kawahaku` (埼玉県立川の博物館), `kagakukan` (さいたま市青少年宇宙科学館), `bonsai-museum` (大宮盆栽美術館) and `meiji-sakado` (明治なるほどファクトリー坂戸) also have no `images[]` photos yet, same as `granemio`/`zouhei`/`kingqueen`/`sayamaski` — the placeholder SVG (`IMG_PLACEHOLDER`) is shown for those until real photos are added. `meiji-sakado` additionally needs a real reservation booked ~30 days ahead (see its `feeReservation`/`tips`) before it can actually be used as a day's plan.
